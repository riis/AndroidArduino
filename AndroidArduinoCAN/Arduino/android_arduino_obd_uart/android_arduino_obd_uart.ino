...
#define ABS_THROTTLE_C   "48"
#define ACC_PEDAL_POS_D  "49"
#define ACC_PEDAL_POS_E  "4A"
#define ACC_PEDAL_POS_F  "4B"
...
void setup() {
  ...
  Serial.println("Connection established, starting the monitor");
}
...
void loop() {
  ...
  Serial.print('\f');
//  requestDataAndSend(ENGINE_LOAD);
  ...
  requestDataAndSend(ABS_THROTTLE_C);
//  requestDataAndSend(ACC_PEDAL_POS_D);
//  requestDataAndSend(ACC_PEDAL_POS_E);
//  requestDataAndSend(ACC_PEDAL_POS_F);
...
void actOnLine(char line[256], int lineLen, String data) {
  if(charArrayEquals(line, lineLen, "01 0C\r") ||
...
void parseDataAndReport(String data, byte bytes[8]) {
  float engineData = calculatePIDvalue(data, bytes);
  String dataString = data + "~" + floatToString(engineData, 3);
  bluetooth.sendStringWithFlags(dataString);
  Serial.println(dataString);
}

float calculatePIDvalue(String PID, byte bytes[8]) {
  if(PID == ENGINE_LOAD)       // (A * 100) / 255
    return (bytes[2] * 100) / 255;

  else if(PID == ENGINE_TEMP)   // (A - 40)
    return (bytes[2] - 40);
  ...
  else if(PID == ABS_THROTTLE_C)        // (A * 100) / 255
    return (bytes[2] * 100) / 255;

  else if(PID == ACC_PEDAL_POS_D)       // (A * 100) / 255
    return (bytes[2] * 100) / 255;

  else if(PID == ACC_PEDAL_POS_E)       // (A * 100) / 255
    return (bytes[2] * 100) / 255;

  else if(PID == ACC_PEDAL_POS_F)       // (A * 100) / 255
    return (bytes[2] * 100) / 255;
